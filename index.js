...
function MenuElemaniOlustur(isim, fiyat, kategori) {
  const menu = { isim, fiyat, kategori };
  return menu;
}

console.log(MenuElemaniOlustur("Kebap", 100, "Anayemek"));
...
console.log(MenuElemaniOlustur("Kurufasulye", 50, "SıcakYemekler"));
console.log(MenuElemaniOlustur("Balık", 75, "DenizMahsul"));
console.log(MenuElemaniOlustur("Kereviz", 66, "Sebze"));
...
degerlendirmeler[7].geribildirim = "şahane bir mekan, fiyatlar gayen uygun";
...
function SonDegerlendirmeyiAl(liste) {
  return `${liste[liste.length - 1].isim} isimli kişi ${
    liste[liste.length - 1].puan
  } puan verdi ve şunları yazdı: ${liste[liste.length - 1].geribildirim}`;
}
